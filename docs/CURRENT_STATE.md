...
| Development phase | M10 remains the latest accepted development milestone; RC31 (`0.1.0-rc.31`) is the current public prerelease on npm `next`; `latest` remains `0.1.0-rc.17`; RC30 remains a prior public prerelease; none of these are stable/production-ready |
...
| RC31 release line | `0.1.0-rc.31` — Complete Pi-Native Streaming, Reliable Mission Cancellation, ADR-048 read-only timeout salvage, and Hardened Ownership Lifecycle; public prerelease on npm `next`; source `132857f62a0e4948c28a89fccbd5ac5111dcb484`, tag `v0.1.0-rc.31`, artifact SHA-256 `935d12caaa957f81dc9c27e00612048a33186dcfb8400cd6e156c3395c58b09c`; live fresh Mission `mission-f36edb90-5681-42de-9a1d-0dee825e4f8b` COMPLETED with M7 pass; cancellation Mission `mission-6de0e5c7-209c-4261-8322-aad4862e0694` CANCELLED with lease released |
...
- **Publication boundary:** do not publish, tag, or GitHub-release RC31
  in this mission. Unpublished candidate:
...
Correction 2026-08-17: RC30 is the current public prerelease
...
ID `f57a8c2b-6d55-4347-8fa7-89b11a81230b`. GitHub prerelease
[v0.1.0-rc.30](https://github.com/Recoba86/PiMultiOrchestrator/releases/tag/v0.1.0-rc.30).
Review bundle SHA-256
`68f77540a2faec88f6ae6c3948cf3a028cfe06bccebaee0e651690e423743147`.
RC29 was not published. RC28 history was not rewritten. The original
source-handoff below is retained; it is not rewritten.

## RC30 Autonomous Boss-led mutation recovery (LOCAL UNPUBLISHED CANDIDATE)

- **Status:** `LOCAL UNPUBLISHED CANDIDATE`. Development manifest is
  `pi-multi-orchestrator@0.1.0-rc.30`. Not public, tagged, npm-published,
  accepted, or a GitHub Release. The frozen RC29 artifact
  `pi-multi-orchestrator-0.1.0-rc.29.tgz` SHA-256
  `fec5a819fd6ae149296bd4328924862abde3defe35893505ba8fbb046dc29f7b` was not
  modified.
- **ADR-050:** local observable Implementation mutation without a valid
  structured result enters Boss-led recovery on the same Task. ADR-049
  CLASS A (`mutation=false`) fallback is unchanged. Unsafe/unknown external
  side effects still require a human.
- **Tests:** `385/385 PASS`; `npm run check` PASS; `git diff --check` PASS;
  `test/mutation-recovery.test.ts` 18 cases.
- **Controlled recovery dogfood (installed `0.1.0-rc.30`, fake executor):**
  fixture `/tmp/pmo-rc30-recovery-dogfood.mjs` against
  `~/.pi/agent/pmo-rc30-local-package`. Mission
  `mission-0a46605a-a68a-49d6-bfbf-067737100cb0` / Task
  `task-f5733d75-22ef-474e-b1c1-d8a94caf1b43` terminal `COMPLETED`. Failed
  Attempt `attempt-3ad4b92c-f9d4-4e83-8db4-a63616886012` remained
  `attempt_interrupted`. Continuation Attempt
  `attempt-5c818daf-8ffb-42f8-883c-e04da3310966` succeeded from the current
  worktree (`REPAIR_EXISTING_WORK`). Evidence: `recovery-assessment`,
  `implementation-result`, `other`. Quality `passed`. Helper preserved;
  remaining wiring completed. This is the recovery proof.
- **Normal real implementation Mission (2026-08-16):**
  `mission-2b90e51e-ce8c-4c1f-8904-491a298d4a64` terminal `COMPLETED` in
  disposable sandbox `/private/tmp/pmo-rc30-impl-r1DnkO`. Boss
  `ag/gemini-3.7-flash-high`, `protocolFailures=0`. Task
  `task-6d878cfc-2062-4617-8047-33fe52871e30` `execution_completed` /
  quality `passed`. Implementation Attempt
  `attempt-f1a0b161-987c-40f3-982c-852d0e566552` on
  `ag/claude-opus-4-6-thinking` submitted a structured result after local
  mutation (`src/greet.ts` + `src/index.ts` re-export). Evidence 1 accepted
  (`evidence-6242f092-fefe-438c-9ae9-719221cdb094`). M7
  `verification-bbc358b1-a052-48de-91d9-7eca42f28a67` on
  `ocg/deepseek-v4-pro`, verdict `pass`. Natural mutation recovery was not
  required. Prior sandbox Mission
  `mission-33b25578-51da-4811-9f76-203740f962e7` stopped at
  `AWAITING_USER` after a continuation `find` hit `.git`; recovery
...
- **Status:** `UNPUBLISHED CANDIDATE / LIVE MISSION COMPLETED`. Development
  identity at the time of the candidate was `pi-multi-orchestrator@0.1.0-rc.29`.
  The artifact is frozen and must not be modified. This is not a public
...
- **Dogfood evidence (public RC28 runtime, live MissionStore snapshot):**
...
- **Worker infrastructure fallback forensics (2026-08-16):** live Mission
  `mission-b290a07a-dba3-4d03-87cf-ebe99dd9e6ae` Attempt
  `attempt-0c01025f-db11-4afc-9a64-67dc9918d54a` on `ocg/gpt-5.6-luna` ended
  `infrastructure_stopped` / mutation 0 in 2723 ms with no second Attempt.
  HealthStore classified `transport_error` (no HTTP status). M4
  `decideFailureAction` returned STOP because live
  `routing.fallback.enabled` is false and `routing.maxAttempts` is 1.
  DeepSeek and Gemini Investigation members were eligible. This matches
...
- **Publication boundary:** public RC28 remains immutable. RC29 is not tagged,
  ...
  required before calling RC29 pre-release ready.

## RC28 public prerelease (immutable baseline)

Correction 2026-08-16: RC28 is a public immutable prerelease
...
`0.1.0-rc.17`. Do not retag, rebuild, or republish RC28. The original
source-handoff text below is retained; it is not rewritten.

## RC28 implemented / pre-release ready

- **Status:** `IMPLEMENTED / PRE-RELEASE READY`. Source is prepared as
  `pi-multi-orchestrator@0.1.0-rc.28`. This is not a public prerelease, npm
  publication, GitHub Release, or accepted development milestone.
- **Dogfood evidence (public RC27, not rewritten):** Mission
  `mission-89d5e163-17ee-4218-b06c-dea5fa4b480b` pinned `cu/cursor-grok-4.6-high`,
  persisted 11 labelled Goal criteria, then exhausted 4 protocol cycles with
  0 tasks after the live Boss adapter received no usable assistant text
  (`Boss provider returned no decision`), terminal `AWAITING_USER`, fallback
  none. Mission `mission-aa30ed69-3213-4cf0-882a-a60be426412d` selected and
  pinned `Tabi/claude-opus-5-thinking` (weight 1, share 100%) then blocked at
  cycle 0 with `Boss infrastructure is unavailable and no configured fallback
  remains` after the original structured invocation failure was discarded and
  the weight-0 Cursor route could not be used as infrastructure fallback.
  These are observed route/runtime compatibility failures, not bootstrap or
  user-configuration regressions, and not a claim about a named vendor.
- **Runtime:** one canonical Boss-response normalization layer accepts Pi
  `AssistantMessage` shapes; user-visible text is only `type:"text"`; `length`
  with a complete decision is usable; failures keep stage/class/stopReason;
  scheduling eligibility ≠ fallback eligibility ≠ protocol validity; weight 0
  remains excluded from normal assignment and may still infrastructure-fallback;
  Inspect and terminal reasons are classified rather than a black box; Boss
  Profile UI distinguishes scheduled Boss, editor selection, and fallback
...
Correction 2026-08-16: RC27 is the public immutable prerelease baseline
(`pi-multi-orchestrator@0.1.0-rc.27`, source
`267612d15dcc0784856e7dafd6704d2f802272b9`, artifact SHA-256
`d8589943434a6ea1796f2c908fa2123464f7c8accca0557bb6547338bea83a55`). The
original source-handoff text below is retained; it is not rewritten. RC28 is
the subsequent public immutable prerelease; RC29 is the current development
line.
