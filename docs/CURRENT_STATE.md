...
| Development phase | M10 accepted; RC16 final external review PASS on an exact detached local candidate; final Planner/manual acceptance hard-blocked by the current live canonical M7 structured-result gate; publication gates pending |
...
| M12 final candidate | `0.1.0-rc.16` — current local candidate; exact artifact and bundle identity are generated by the detached verifier |
| RC16 final Planner attempt | 2026-08-15 — hard-blocked at live canonical M7 reviewer submission; source-bound artifact remains unchanged |
