...
- **Decision:** When the bounded finalizer also fails to capture a valid result, keep backward-compatible `invalid_child_result` (or `partial_mutation_requires_review` after mutation) and attach a precise in-memory `resultFinalization` diagnostic. Do not add a persisted terminal-state enum. ADR-049 supersedes the hard stop for non-mutating misses after that turn.
