...
- **Decision:** Package only the compiled extension entrypoint and declarations as `0.1.0-rc.1` with Pi `0.84.x` as a peer dependency. Generate the artifact, checksum, file list, manifest, unpacked verification, compatibility/checklist/dogfood records, and external-review bundle outside the checkout. Do not publish, tag, push, call live routes, or alter live Pi state.
- **Rationale:** Pi's supported `pi-package` contract provides reproducible local installation while a strict allowlist prevents state, secrets, source paths, and development dependencies from shipping. A rescue path that can remove/restore the package without importing the extension limits blast radius.
