...
| Stage 1 — artifact-derived isolated install | PASS (directory-source workflow) | Candidate `0.1.0-rc.1`, SHA-256 `b048a0c9a50c2283ffc69f52f728ac8abb3afd8150ab8bfe683a8a46a7c5fab2`; checksum-verified `.tgz` extracted to a fresh temporary `package/` directory, then `pi install <package-dir>`/`pi list` in isolated settings. Direct `.tgz` installation is unsupported by Pi `0.84.1`. |
| Stage 2 — normal non-destructive Control Center | PASS (directory-source workflow) | The artifact-derived installed directory loaded `/orchestrator` and Diagnostics with candidate version/schema metadata and all twelve sections; fake gateway/temporary roots only |
