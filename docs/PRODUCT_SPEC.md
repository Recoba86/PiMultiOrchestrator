...
- **BOSS-010:** Live Boss inference MUST normalize Pi `completeSimple` / `AssistantMessage` responses through one canonical layer: user-visible `type:"text"` content only, never hidden thinking; successful-enough stop reasons include `stop` and `length` when a complete decision exists. Failures MUST be classified by stage and class (route-resolution, capability, request, response, decision-protocol) with Inspectable safe diagnostics. Empty text, truncated output, unsupported shape, refusal, cancellation, and transport/provider failure MUST remain distinguishable. Empty text is delivery/infrastructure; truncated incomplete JSON and invalid BossDecision documents remain protocol. Secrets, authorization material, unrestricted provider payloads, and hidden reasoning MUST NOT be persisted.
- **BOSS-011:** Before Boss inference, Goal and Mission acceptance criteria that require worker-impossible operations (including `git commit`, `git push`, and other network/publication commands) MUST terminal `AWAITING_USER` with provenance `CAPABILITY_MISMATCH`. Implementation workers MUST NOT receive `explicitlyAuthorized` for those commands. Logical Tasks MUST be reused by `executionClass` plus normalized objective (or an existing Mission `taskId`); historical cancelled or superseded rows remain durable and MUST NOT poison completion. COMPLETED requires Boss `complete` with `acceptanceSatisfied` plus every **active** Task `execution_completed` and M7 `passed`. Invocation-delivery exhaustion is `BLOCKED`; decision-protocol exhaustion with no Tasks remains `AWAITING_USER`. The Boss prompt MUST include a bounded canonical Mission projection, not raw transcripts or hidden reasoning.
...
- **FB-001:** The failure classifier MUST distinguish quota exhausted, rate limited, authentication failed, timeout, transport failure, model unavailable, provider unavailable, cancelled, unknown, and worker result-capability failure.
...
- **GATE-003:** Only the Boss may transition a mission to completed after all required gates pass or are validly waived. Required execution/quality gates apply to **active** Tasks (latest non-cancelled identity per execution class and normalized objective). Historical cancelled or superseded Task rows remain durable evidence and are not themselves completion requirements.
