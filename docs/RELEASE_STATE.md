...
| Development manifest version | `0.1.0-rc.2` — local release candidate only |
| Release tag | NONE |
| Release commit | NONE |
| GitHub release | NONE |
...
| M11-R4 candidate | `0.1.0-rc.2`; `165/165 PASS`, release subtests `4/4 PASS`, Pi `0.84.1` directory-source install, all-twelve Control Center, seeded M10 compatibility upgrade/rollback/rescue, privacy scan, and self-contained bundle verification PASS; exact artifact/source/build identities are in the generated manifest; external review and Planner acceptance pending |
| Typecheck | PASS |
...
- R4 evidence is generated from a clean commit with `dirty: false`; the manifest binds source/tree/build digests and trusted executable identities. The review bundle contains the rc.2 artifact/checksum, directory source, clean privacy report, `165/165` test evidence, real M10 baseline commit `c65470c001e539c36f0a53cacd912f48eb05ff7f`, Pi evidence, seeded-state hashes, rescue evidence, and deterministic verification. It remains external-review evidence, not acceptance or publication.
