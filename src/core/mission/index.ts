...
export class SQLiteMissionStore implements MissionStoreAdapter {
	...
	setTaskQualityStatus(input: TaskQualityStatus): TaskQualityStatus { return this.tx(() => { this.missionRow(String(input.missionId)); if (!this.db.prepare("SELECT 1 FROM tasks WHERE task_id=? AND mission_id=?").get(String(input.taskId), String(input.missionId))) throw new MissionNotFoundError("task", String(input.taskId)); if (!Number.isSafeInteger(input.qualityRound) || input.qualityRound < 0) throw new MissionValidationError([{ path: "qualityRound", message: "quality round is invalid" }]); if (!["unverified", "verification_running", "passed", "rejected", "blocked", "review_required"].includes(input.status)) throw new MissionValidationError([{ path: "status", message: "quality status is invalid" }]); const at = this.text(input.updatedAt, "updatedAt"); this.db.prepare("INSERT INTO task_quality_status(task_id,mission_id,status,quality_round,latest_verification_id,latest_decision_id,updated_at) VALUES (?,?,?,?,?,?,?) ON CONFLICT(task_id) DO UPDATE SET mission_id=excluded.mission_id,status=excluded.status,quality_round=excluded.quality_round,latest_verification_id=excluded.latest_verification_id,latest_decision_id=excluded.latest_decision_id,updated_at=excluded.updated_at").run(input.taskId, input.missionId, input.status, input.qualityRound, input.latestVerificationId ?? null, input.latestDecisionId ?? null, at); this.event(String(input.missionId), Number(this.missionRow(String(input.missionId)).revision), "quality_status_changed", "system", { taskId: String(input.taskId), status: input.status, qualityRound: input.qualityRound }); return this.getTaskQualityStatus(String(input.taskId))!; }); }
	...
	acquireLease(missionId: string, owner: string, options: { readonly ttlMs?: number; readonly forceRecover?: boolean } = {}): LeaseRecord { return this.tx(() => { this.missionRow(missionId); if (!owner.trim()) throw new MissionLeaseError("lease owner is required"); const ttl = options.ttlMs ?? this.leaseTtlMs; if (!Number.isSafeInteger(ttl) || ttl <= 0) throw new MissionLeaseError("lease ttl must be positive"); const existing = this.db.prepare("SELECT * FROM mission_leases WHERE mission_id=?").get(missionId) as Row | undefined; const now = this.clock(); const wasActive = existing && new Date(String(existing.expires_at)).getTime() > now.getTime(); if (wasActive && String(existing?.owner) !== owner && !options.forceRecover) throw new MissionLeaseError("mission is leased by another owner"); const expires = new Date(now.getTime() + ttl).toISOString(); const recoveredFrom = existing?.owner && String(existing.owner) !== owner ? String(existing.owner) : null; this.db.prepare("INSERT OR REPLACE INTO mission_leases(mission_id,owner,acquired_at,heartbeat_at,expires_at,recovered_from) VALUES (?,?,?,?,?,?)").run(missionId, owner, now.toISOString(), now.toISOString(), expires, recoveredFrom); if (recoveredFrom) this.event(missionId, Number(this.missionRow(missionId).revision), "lease_recovered", "system", { recoveredFrom, owner }); return { missionId: missionId as MissionId, owner: owner as LeaseOwner, acquiredAt: now.toISOString(), heartbeatAt: now.toISOString(), expiresAt: expires, ...(recoveredFrom ? { recoveredFrom: recoveredFrom as LeaseOwner } : {}) }; }); }
	heartbeatLease(missionId: string, owner: string, ttlMs = this.leaseTtlMs): LeaseRecord { return this.tx(() => { const existing = this.db.prepare("SELECT * FROM mission_leases WHERE mission_id=? AND owner=?").get(missionId, owner) as Row | undefined; if (!existing) throw new MissionLeaseError("lease not held"); if (!Number.isSafeInteger(ttlMs) || ttlMs <= 0) throw new MissionLeaseError("lease ttl must be positive"); const now = this.clock(); const expires = new Date(now.getTime() + ttlMs).toISOString(); this.db.prepare("UPDATE mission_leases SET heartbeat_at=?,expires_at=? WHERE mission_id=? AND owner=?").run(now.toISOString(), expires, missionId, owner); return { missionId: missionId as MissionId, owner: owner as LeaseOwner, acquiredAt: String(existing.acquired_at), heartbeatAt: now.toISOString(), expiresAt: expires }; }); }
	releaseLease(missionId: string, owner: string): void { this.tx(() => { this.db.prepare("DELETE FROM mission_leases WHERE mission_id=? AND owner=?").run(missionId, owner); }); }
