...
import { MISSION_STORE_SCHEMA_SQL, CURRENT_MISSION_STORE_SCHEMA_VERSION } from "./schema.js";
import {
...
export class SQLiteMissionStore implements MissionStoreAdapter {
	...
	constructor(options: MissionStoreOptions) {
		...
		try {
			...
			if (meta && Number(meta.value) !== CURRENT_MISSION_STORE_SCHEMA_VERSION) throw new MissionVersionError(Number(meta.value), CURRENT_MISSION_STORE_SCHEMA_VERSION);
			this.db.prepare("INSERT OR IGNORE INTO mission_store_meta(key,value) VALUES ('schema_version',?)").run(String(CURRENT_MISSION_STORE_SCHEMA_VERSION));
			this.integrityCheck();
		} catch (error) {
	...
	recordCheckpoint(missionId: string, kind: CheckpointKind = "manual"): CheckpointRecord { return this.tx(() => { const mission = this.missionFrom(this.missionRow(missionId)); const id = `checkpoint-${this.makeId()}` as CheckpointRecord["checkpointId"]; const at = nowIso(this.clock); this.db.prepare("INSERT INTO mission_checkpoints(checkpoint_id,mission_id,revision,kind,status,snapshot_json,created_at) VALUES (?,?,?,?,?,?,?)").run(id, missionId, mission.revision, kind, mission.status, this.snapshot(mission), at); this.event(missionId, mission.revision, "checkpoint_created", "system", { checkpointId: id }); return { checkpointId: id, missionId: missionId as MissionId, revision: mission.revision, kind, status: mission.status, snapshot: mission, createdAt: at }; }); }
