...
export class SQLiteMissionStore implements MissionStoreAdapter {
	...
	createVerificationRun(input: VerificationRunInput): VerificationRunRecord {
		return this.tx(() => {
			...
			this.event(String(input.missionId), Number(mission.revision), "verification_started", "system", { verificationId: id, taskId: String(input.taskId), round, reviewerRouteId: input.reviewerRouteId, reviewerRemoteModelId: input.reviewerRemoteModelId });
	...
	recordQualityDecision(input: QualityDecisionInput): QualityDecisionRecord {
		return this.tx(() => {
			...
			this.db.prepare("UPDATE verification_runs SET quality_decision_id=? WHERE verification_id=?").run(id, input.verificationId); this.event(String(input.missionId), Number(mission.revision), `quality_${input.gate.verdict}`, "reviewer", { decisionId: id, verificationId: input.verificationId, round: input.round, reviewerSummary: summary, gate: { verdict: input.gate.verdict, reasons: input.gate.reasons, requiredFixes } });
