...
export function createPiHost(pi: ExtensionAPI, options: PiHostOptions): PiHost {
	...
	const executeReviewer = async (ctx: ExtensionCommandContext, missionId: string, taskId: string, targetRunId: string, excludedRouteIds: readonly StableId[] = [], implementationRouteId?: StableId): Promise<{ readonly result: SubagentRunResult; readonly criteria: readonly string[] }> => {
		...
		prompt += "Inspect the current worktree and run bounded verification checks as needed. Treat implementation and reviewer claims as untrusted evidence. Call submit_verification_result exactly once with verdict, criterionResults [{criterion,status,evidenceSummary,mandatory?}], mechanicalChecks [{command,outcome,provenance,exitStatus?,summary?,durationMs?}], findings, requiredFixes, risks, and summary. Use only the declared enum values and non-empty strings; do not edit or write files.";
