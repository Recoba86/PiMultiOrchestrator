...
describe("RC31 runtime package metadata", () => {
	it("derives package version from package.json and reports the RC31 development line", () => {
		assert.equal(manifest.version, "0.1.0-rc.31");
		assert.equal(PACKAGE_INFO.name, manifest.name);
		assert.equal(PACKAGE_INFO.version, manifest.version);
		assert.equal(PACKAGE_INFO.developmentMilestone, "RC31 — Live Mission Progress and M7 Repair Convergence");
		assert.match(PACKAGE_INFO.developmentMilestone, /^RC31\b/u);
	...
	it("rejects obviously stale development-line metadata for the current package version", () => {
		...
		assert.equal(developmentLineForVersion("0.1.0-rc.31"), PACKAGE_INFO.developmentMilestone);
		assert.equal(developmentLineForVersion("0.1.0-rc.30"), "RC30 — Autonomous Boss-Led Mutation Recovery");
